...
const fCurrency = (v) => new Intl.NumberFormat('es-PE', { style: 'currency', currency: 'PEN' }).format(v);
    ...
    try {
        ...
        allData = json.map((row) => {
            ...
            return {
                ...
                medico: (row.servicio || row.medico || '').toString().trim().toUpperCase(),
                cod_seg: (row.cod_seg || '').toString().trim(),
                cia: (row.cia || '').toString().trim().toUpperCase(),
                isParticular: row.cia && row.cia.toString().toUpperCase().includes('PARTICULAR'),
                _rawDate: d
            };
        });
        ...
        // Mostrar elementos
        ['filterPanel', 'kpiSection', 'chartMainContainer', 'chartPieContainer', 'tableSection', 'kpiDivider', 'chartDivider', 'tableDivider', 'rankingSection', 'rankingDivider', 'ciaSection', 'ciaDivider', 'dataFooter'].forEach((id) => {
...
function setupFilters() {
    const years = [...new Set(allData.map((d) => d.year))].filter((y) => !isNaN(y)).sort();
...
function applyFilters() {
    const selYears = Array.from(document.querySelectorAll('#yearFilters input:checked')).map((i) => parseInt(i.value));
    const selMonths = Array.from(document.querySelectorAll('#monthFilters input:checked')).map((i) => parseInt(i.value));
    const selArea = document.getElementById('areaSelect').value;

    filteredData = allData.filter((d) => selYears.includes(d.year) && selMonths.includes(d.mes) && (selArea === 'all' || d.area === selArea));
...
function setMetric(m) {
    currentMetric = m;
    document.getElementById('btnMonto').className = m === 'monto' ? 'seg-btn seg-btn-active' : 'seg-btn';
    document.getElementById('btnCant').className = m === 'cantidad' ? 'seg-btn seg-btn-active' : 'seg-btn';
    applyFilters();
}
...
function setPieMetric(m) {
    currentPieMetric = m;
    document.getElementById('btnPieMonto').className = m === 'monto' ? 'seg-btn seg-btn-active' : 'seg-btn';
    document.getElementById('btnPieCant').className = m === 'cantidad' ? 'seg-btn seg-btn-active' : 'seg-btn';
    updatePieChart();
}

function toggleDataLabels() {
    showLabels = !showLabels;
    document.getElementById('btnLabels').className = showLabels ? 'btn-labels btn-labels-active' : 'btn-labels';
    applyFilters();
}
...
function toggleView() {
    ...
    document.getElementById('viewText').textContent = isTableView ? 'Gráfico' : 'Matriz';
...
function updateMainChart(years, months) {
    ...
    const datasets = years.map((y, i) => {
        const data = sortedMonths.map((m) => {
            ...
            return currentMetric === 'monto' ? group.reduce((s, r) => s + r.importe, 0) : group.length;
        });
        return {
            label: `Año ${y}`,
            data,
        ...
        options: {
            ...
            plugins: {
                ...
                datalabels: {
                    ...
                    formatter: (v) => (currentMetric === 'monto' ? `S/ ${Math.round(v / 1000)}K` : fNumber(v))
                }
            },
            scales: {
                y: {
                    ...
                    ticks: {
                        ...
                        callback: (v) => (currentMetric === 'monto' ? `S/ ${v / 1000}K` : v)
                    }
                },
                x: {
                    grid: { display: false },
...
function updateMatrixTable() {
    ...
    const selMonths = Array.from(document.querySelectorAll('#monthFilters input:checked'))
        .map((i) => parseInt(i.value))
        .sort((a, b) => a - b);
    const years = [...new Set(filteredData.map((d) => d.year))].filter((y) => !isNaN(y)).sort();
    const head = document.getElementById('matrixHead');
    const body = document.getElementById('matrixBody');

    let headHTML = `<tr><th class="px-8 py-5 text-left">Periodo</th>`;
    years.forEach((y) => (headHTML += `<th class="px-8 py-5 text-right">${y}</th>`));
    headHTML += `</tr>`;
    ...
    selMonths.forEach((m) => {
        ...
        years.forEach((y) => {
            const val = filteredData.filter((d) => d.year === y && d.mes === m).reduce((s, r) => s + (currentMetric === 'monto' ? r.importe : 1), 0);
...
function updatePieChart() {
    ...
    if (currentPieMetric === 'monto') {
        pVal = filteredData.filter((d) => d.isParticular).reduce((s, r) => s + r.importe, 0);
        sVal = filteredData.filter((d) => !d.isParticular).reduce((s, r) => s + r.importe, 0);
    } else {
    ...
    if (donutTotal && donutLabel) {
        donutTotal.textContent = currentPieMetric === 'monto' ? fCurrency(total) : fNumber(total);
        donutLabel.textContent = currentPieMetric === 'monto' ? 'Facturación' : 'Atenciones';
    }
...
function updateTable() {
    ...
    const totals = areas.map((a) => filteredData.filter((d) => d.area === a).reduce((s, r) => s + r.importe, 0));
    const maxTotal = Math.max(...totals);

    areas.forEach((a, idx) => {
        const rows = filteredData.filter((d) => d.area === a);
        const total = totals[idx];
        const part = rows.filter((d) => d.isParticular).reduce((s, r) => s + r.importe, 0);
...
function setRankingMetric(m) {
    currentRankMetric = m;
    document.getElementById('btnRankMonto').className = m === 'monto' ? 'seg-btn seg-btn-active !text-[10px] !px-3 !py-1.5' : 'seg-btn !text-[10px] !px-3 !py-1.5';
    document.getElementById('btnRankCant').className = m === 'cantidad' ? 'seg-btn seg-btn-active !text-[10px] !px-3 !py-1.5' : 'seg-btn !text-[10px] !px-3 !py-1.5';
    updateRanking();
}

function setRankingMetricEspec(m) {
    currentRankMetricEspec = m;
    document.getElementById('btnRankMontoEspec').className = m === 'monto' ? 'seg-btn seg-btn-active !text-[10px] !px-3 !py-1.5' : 'seg-btn !text-[10px] !px-3 !py-1.5';
    document.getElementById('btnRankCantEspec').className = m === 'cantidad' ? 'seg-btn seg-btn-active !text-[10px] !px-3 !py-1.5' : 'seg-btn !text-[10px] !px-3 !py-1.5';
    updateRankingEspec();
}

function getRankBadge(idx) {
    if (idx === 0) return '<span class="bg-amber-100 text-amber-600 font-black text-[10px] px-2 py-1 rounded-full border border-amber-200">#1 TOP</span>';
    if (idx === 1) return '<span class="bg-slate-200 text-slate-500 font-black text-[10px] px-2 py-1 rounded-full border border-slate-300">#2</span>';
    if (idx === 2) return '<span class="bg-orange-100 text-orange-600 font-black text-[10px] px-2 py-1 rounded-full border border-orange-200">#3</span>';
...
function updateRanking() {
    ...
    const docNames = typeof MEDICOS !== 'undefined' ? MEDICOS.map((m) => m.nombre.toUpperCase()) : [];
    ...
    filteredData.forEach((row) => {
        ...
        if (stats[med]) {
            ...
            const valid = codSeg === '50.00.01' || codSeg === '50.02.03' || codSeg === '50.00.00';
    ...
    ranking.forEach((doc, idx) => {
        ...
        const valText = currentRankMetric === 'monto' ? fCurrency(doc.monto) : fNumber(doc.cantidad);
        const secValText = currentRankMetric === 'monto' ? `${fNumber(doc.cantidad)} atenciones` : `Facturado: ${fCurrency(doc.monto)}`;
        const pct = maxVal > 0 ? ((doc[currentRankMetric] / maxVal) * 100).toFixed(1) : 0;
...
function updateRankingEspec() {
    ...
    const docNamesGuardia = typeof MEDICOS !== 'undefined' ? MEDICOS.map((m) => m.nombre.toUpperCase()) : [];
    ...
    filteredData.forEach((row) => {
        ...
        const isEspecCode = codSeg.startsWith('50.') || ['00.17.25', '00.18.90', '00.20.38', '00.17.01', '00.17.04'].includes(codSeg);

        if (!isEspecCode) return;

        // Verificar si esta atención YA pertenece al ranking de Guardia
        const isGuardiaAttn = docNamesGuardia.includes(med) && ['50.00.01', '50.02.03', '50.00.00'].includes(codSeg);
    ...
    ranking.forEach((doc, idx) => {
        ...
        const valText = currentRankMetricEspec === 'monto' ? fCurrency(doc.monto) : fNumber(doc.cantidad);
        const secValText = currentRankMetricEspec === 'monto' ? `${fNumber(doc.cantidad)} atenciones` : `Facturado: ${fCurrency(doc.monto)}`;
        const pct = maxVal > 0 ? ((doc[currentRankMetricEspec] / maxVal) * 100).toFixed(1) : 0;
        const rankBadge = getRankBadge(idx);
...
function setRankingMetricCia(m) {
    ...
    document.getElementById('btnRankMontoCia').className = m === 'monto' ? 'seg-btn seg-btn-active !text-[10px] !px-3 !py-1.5' : 'seg-btn !text-[10px] !px-3 !py-1.5';
    document.getElementById('btnRankCantCia').className = m === 'cantidad' ? 'seg-btn seg-btn-active !text-[10px] !px-3 !py-1.5' : 'seg-btn !text-[10px] !px-3 !py-1.5';
    updateRankingCia();
}
...
function updateRankingCia() {
    ...
    ranking.forEach((doc, idx) => {
        ...
        const valText = currentRankMetricCia === 'monto' ? fCurrency(doc.monto) : fNumber(doc.cantidad);
        const secValText = currentRankMetricCia === 'monto' ? `${fNumber(doc.cantidad)} atenciones` : `Facturado: ${fCurrency(doc.monto)}`;
        const pct = maxVal > 0 ? ((doc[currentRankMetricCia] / maxVal) * 100).toFixed(1) : 0;
        const rankBadge = getRankBadge(idx);
