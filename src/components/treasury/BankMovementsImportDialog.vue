...
const downloadTemplate = async () => {
    try {
        const response = await TreasuryService.downloadImportTemplate();
        const contentType = response.headers?.['content-type'] || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
        const blob = new Blob([response.data], { type: contentType });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'plantilla_movimientos_bancarios.xlsx';
        a.click();
...
const buildFormData = (dryRun) => {
    ...
    fd.append('bank_account_id', importAccountId.value);
    fd.append('dry_run', dryRun ? 'true' : 'false');
    if (importYear.value) fd.append('year', importYear.value);
    return fd;
};
...
const handleValidate = async () => {
    if (!importAccountId.value) {
        toast.add({ severity: 'warn', summary: 'Requerido', detail: 'Seleccione una cuenta bancaria', life: 3000 });
        return;
    }
    if (!importFile.value) {
    ...
    loading.value = true;
    try {
        const res = await TreasuryService.importBankMovements(buildFormData(true));
        dryResult.value = {
            canImport: true,
        ...
        // 422: backend devolvió errores de validación bloqueantes
...
const handleConfirm = async () => {
    loading.value = true;
    try {
        const res = await TreasuryService.importBankMovements(buildFormData(false));
        doneResult.value = {
        ...
        toast.add({ severity: 'error', summary: 'Error al importar', detail: err?.message || 'No se pudieron importar los movimientos', life: 4000 });
    } finally {
        loading.value = false;
    }
};
...
<template>
    <Dialog v-model:visible="visible" :style="{ width: '620px' }" :modal="true" :closable="true" class="import-dialog" @hide="handleClose">
        <!-- ═══ HEADER ═══ -->
        <template #header>
            <div class="dialog-header">
                <div class="dialog-header-icon import-icon">
                    <i class="pi pi-upload"></i>
                </div>
                <div>
                    <h3 class="dialog-title">
                        {{ step === 'setup' ? 'Importación Masiva' : step === 'preview' ? 'Vista Previa' : 'Importación Completada' }}
                    </h3>
                    <p class="dialog-subtitle">
                        {{ step === 'setup' ? 'Carga movimientos bancarios desde un archivo Excel' : step === 'preview' ? 'Revisa el resultado antes de confirmar' : 'Los movimientos fueron registrados exitosamente' }}
                    </p>
                </div>
            </div>
        </template>
        ...
        <div v-if="step === 'setup'" class="dialog-body">
            ...
            <div class="section-divider">
                <span class="section-divider-label"><i class="pi pi-sliders-h mr-1"></i>Configurar importación</span>
            </div>
            ...
            <div class="form-field">
                ...
                <div class="file-upload-zone" :class="{ 'has-file': importFile }" @click="fileInputRef.click()">
                    ...
                    <template v-else>
                        ...
                        <span class="file-zone-hint-sub">Formatos: .xlsx, .xls</span>
                    </template>
                </div>
            </div>
        ...
        <div v-else-if="step === 'preview'" class="dialog-body">
            ...
            <div v-if="dryResult.canImport && dryResult.errors.length === 0 && dryResult.warnings_detail.length === 0" class="all-clear-banner">
                ...
                <div>
                    ...
                    <span class="all-clear-sub">Las {{ dryResult.valid }} filas se importarán sin inconvenientes.</span>
                </div>
            </div>
        </div>
...
<style scoped>
/* ── Dialog shell ── */
:deep(.import-dialog .p-dialog-header) {
...
:deep(.import-dialog .p-dialog-footer) {
    padding: 0;
